...
{
  V8_STUFF();
  edict_t *entity = (*g_engfuncs.pfnFindEntityByString)(
      structures::unwrapEntity(isolate, info[0]),
      utils::js2string(isolate, info[1]),
      utils::js2string(isolate, info[2]));
  
  // ignore worldspawn
  if (!entity || (*g_engfuncs.pfnIndexOfEdict)(entity) == 0) {
    info.GetReturnValue().Set(v8::Null(isolate));
    return;
  }

  info.GetReturnValue().Set(structures::wrapEntity(isolate, entity));
}
...
void sf_eng_pfnCreateNamedEntity(const v8::FunctionCallbackInfo<v8::Value> &info)
{
  ...
  const char *className = utils::js2string(isolate, info[0]);
  edict_t* entity = (*g_engfuncs.pfnCreateNamedEntity)((*g_engfuncs.pfnAllocString)(className));
  info.GetReturnValue().Set(structures::wrapEntity(isolate, entity));
}
